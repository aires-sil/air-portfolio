...
set "CQ=18"
set "NVENC_PRESET=p5"
set "PIX_FMT=nv12"        :: Use NV12 for NVENC
set "DENISE_STRENGTH=0.3" :: Denoise strength (Vulkan)
set "DEBAND_STRENGTH=3"   :: Deband strength (Vulkan)
...
for /r "%RAW%" %%F in (*.mkv) do (
    ...
    :: === Mirror subfolder structure relative to RAW ===
    set "RELPATH=%%~pF"
    ...
    :: === Auto-select first English/Original audio track ===
    set "AUDIO_INDEX="
    for /f "tokens=1,2 delims=," %%a in ('ffprobe -v error -select_streams a -show_entries stream=index:stream_tags=language -of csv=p=0 "!INFILE!"') do (
        set "IDX=%%a"
        set "LANG=%%b"
        echo !LANG! | findstr /i "Eng English Original" >nul
    ...
    if /i "!COLOR_INFO!"=="bt2020|smpte2084" (
        echo Detected HDR10 PQ → applying Vulkan tonemap
        set "FILTERS=hwupload=derive_device=vulkan,
            scale_vulkan=%RESOLUTION%:force_original_aspect_ratio=decrease,
            tonemap_vulkan=tonemap=%TONEMAP_FILTER%:format=nv12,
            deband_vulkan=strength=%DEBAND_STRENGTH%,
            denoise_vulkan=strength=%DENISE_STRENGTH%"
    ) else if /i "!COLOR_INFO!"=="bt2020|arib-std-b67" (
        echo Detected HDR10 HLG → applying Vulkan tonemap
        set "FILTERS=hwupload=derive_device=vulkan,
            scale_vulkan=%RESOLUTION%:force_original_aspect_ratio=decrease,
            tonemap_vulkan=tonemap=%TONEMAP_FILTER%:format=nv12,
            deband_vulkan=strength=%DEBAND_STRENGTH%,
        ...
        echo Detected SDR → skipping tonemap, using Vulkan scale + filters
        set "FILTERS=hwupload=derive_device=vulkan,
            scale_vulkan=%RESOLUTION%:force_original_aspect_ratio=decrease,
            deband_vulkan=strength=%DEBAND_STRENGTH%,
            denoise_vulkan=strength=%DENISE_STRENGTH%"
    ...
    :: === Run FFmpeg encode with Vulkan → NVENC, fallback to CPU if Vulkan fails ===
    ffmpeg -y -init_hw_device vulkan -filter_hw_device vulkan -i "!INFILE!" ^
        ...
        "!OUTFILE!" 2>nul || (
            echo Vulkan processing failed, falling back to CPU filters...
            ffmpeg -y -i "!INFILE!" ^
                -map 0:v:0 ^
